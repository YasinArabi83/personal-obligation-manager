...
## ADR-0028: Frontend app shell — react-router v7, TanStack Query v5, localStorage tokens with single-flight refresh

- Date: 2026-08-24
- Status: Accepted
- Context: Task 0009 built the first real frontend surface (obligations list/filter/search page) on top of the bare Vite scaffold from 0001. `ARCHITECTURE.md §7` mandated feature folders and React Query for server state but pinned neither a router nor token storage, and although every `/obligations` call requires a JWT, the repository had no frontend auth work scheduled anywhere in `PROGRESS.md`.
- Decision:
  - **Router:** adopt `react-router-dom` v7 with routes `/login` and a protected `/`; filter/pagination state lives in URL query params via `useSearchParams`, so views are shareable and back/forward restores them. This baseline serves tasks 0010–0012 as well.
  - **Server state:** adopt `@tanstack/react-query` v5 with `retry: false` (API errors surface through the standard `{error:{code,message}}` envelope instead of silent retries) and `keepPreviousData` for smooth pagination.
  - **Auth slice (user-approved scope extension):** task 0009 includes a minimal phone+OTP login page consuming the existing `auth/*` endpoints; tokens and user are stored in `localStorage`. No form library yet — `react-hook-form`+Zod arrive with task 0010's forms.
  - **Single-flight refresh:** the shared `apiClient` attaches the Bearer header and, on `401`, runs exactly one `POST /auth/refresh` shared across all concurrent requests, retrying each original request once. This is mandatory given ADR-0017's replay detection: parallel refreshes would consume rotated tokens and revoke the entire family. Unrecoverable expiry clears storage, notifies registered listeners, and redirects to `/login`.
  - **Jalali deferred:** dates render Gregorian `YYYY-MM-DD` through one shared helper; the Jalali component remains task 0011's scope.
- Consequences: XSS could read `localStorage` tokens — accepted for the single-VPS MVP; moving the access token into an httpOnly cookie requires its own ADR plus CSRF handling. Login UX intentionally surfaces server rate-limit envelopes verbatim (including `Retry-After`) rather than inventing client-side throttling. Separately, the accidentally-committed empty root `package.json` broke vitest/esbuild config resolution walking up from `src/Web`; it now contains valid `{}` — deciding between deleting it and making it a real workspace manifest stays open.
