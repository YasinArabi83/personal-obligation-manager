...
> **POM override (Persian RTL product):** the Latin pairing below does not cover Persian script.
> POM uses **Vazirmatn** (100–900 variable weights) as the single UI face, **self-hosted via the
> npm `vazirmatn` package** — never a Google Fonts CDN request at runtime, per the project's
> sanctions/filtering constraint for Iranian users (`AGENTS.md §2`). Wired in `src/Web/src/index.css`
> (font-family token in `tailwind.config.ts`).

- **Heading Font:** Lexend *(superseded by Vazirmatn bold weights)*
- **Body Font:** Source Sans 3 *(superseded by Vazirmatn regular)*
